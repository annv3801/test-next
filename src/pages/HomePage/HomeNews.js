...
async function getData() {
    try {
        const res = await axios.post('http://localhost:4444/LiquorExchange/News/Get-List-News', {
                pageSize: 4,
                currentPage: 1,
                searchByFields: [],
                sortByFields: [],
                    ...
                    {news.map((newsItem) => {
                        ...
                        return (
                            <a href={`/tin-tuc/${newsItem.slug}`} className="bg-white px-2 py-2 md:px-3 md:py-3 flex flex-col rounded-xl hover:border-blue-500 hover:text-blue-500 duration-200 ease-in-out">
                                <img className="rounded-xl" src={`http://localhost:4444/Uploads/${newsItem.image}?height=300&width=500`} alt={newsItem.name} title={newsItem.name}/>
