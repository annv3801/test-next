...
export default function HomeCarousel() {
    ...
    return (
        <Carousel autoplay>
            {data.map((s) => {
                return (
                    <img loading="lazy" src={`https://api.ruoudutysanbay.com/Uploads/${s.image}?height=${windowDimensions.height}&width=${windowDimensions.width}`} width={windowDimensions.width} height={windowDimensions.height} className="lg:h-full object-cover" alt={s.name} key={s.id} title={s.name}/>
                );
            })}
        </Carousel>
    );
